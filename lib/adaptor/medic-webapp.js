...
exports.prototype = (function() {
	...
	process_pending_message = function(m) {
		m = translate_for_transmission(m);
	...
	translate_for_transmission = function (m) {
		return {
			uuid: m.uuid,
			content: m.message,
			to: m.to,
	...
	translate_for_delivery = function (m) {
	...
	self.deliver = function(message, callback) {
		var post_url = config.url + '/messages';
		message = translate_for_delivery(message);
		request.post(post_url, message, function() {
		});
	};
