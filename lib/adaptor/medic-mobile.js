...
exports.prototype = {
  ...
  self._debug_print('_poll_for_transmit :: messages: ' + total_messages);
  ...
  _handle_transmit_error: function (_e) {
    ...
    this._debug_print('transmit_handler: Unable to transmit message');
    this._debug_print('transmit error was: ' + JSON.stringify(_e));
...
console.log('HARDCODED GET REQUEST');
 ...
 _transmit_http_callback_autoreplies: function (_callback) {
  ...
  _poll_for_transmit: function (_callback) {
    ...
    http_request.get(request, function (_err, _resp, _body) {
    self._debug_print('_poll_for_transmit() ::::: got response with:');
    self._debug_print('_poll_for_transmit() :::::                _err: ' + _err);
    self._debug_print('_poll_for_transmit() :::::                _resp: ' + _resp);
    self._debug_print('_poll_for_transmit() :::::                _body: ' + _body);
      var rv = null;
      try {
        rv = JSON.parse(_body);
      } catch (_e) {
