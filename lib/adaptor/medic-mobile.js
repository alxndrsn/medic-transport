...
exports.prototype = {
  ...
  deliver: function (_message, _callback) {
    ...
    self._debug_print("deliver() :: making request: " + JSON.stringify(request));
    http_request.post(request, function (_err, _resp, _body) {
      self._debug_print("deliver().post():callback()");
      self._debug_print("  _err=" + _err);
      self._debug_print("  _resp=" + JSON.stringify(_resp));
      self._debug_print("  _body=" + _body);
  ...
  register_transmit_handler: function (_handler) {
    this._debug_print("register_transmit_handler() :: _handler=" + _handler);
   ...
   */
   _invoke_transmit_handler: function (_message, _callback) {
    this._debug_print("_invoke_transmit_handler() :: _message=" + JSON.stringify(_message));
    ...
    this._debug_print("Calling transmit handler...");
  ...
  _handle_transmit_timer: function (_completion_callback) {
    ...
    self._poll_for_transmit(function (_err, _poll_results) {
      self._debug_print("_poll_for_transmit().callback() :: ENTRY");
      ...
      self._debug_print("_poll_for_transmit().callback() :: do we have messages? " + JSON.stringify(messages));
      ...
      async.each(messages,
        ...
        function (_message, _callback) {
          ...
          self._invoke_transmit_handler(message, function (_e, _tx_result) {
            self._debug_print("_invoke_transmit_handler().callback() :: " +
                "_e=" + _e + ", _tx_result=" + _tx_result);
  ...
  _perform_http_callbacks: function (_callback_object, _depth, _callback) {
    ...
    self._debug_print('-----');
    self._debug_print('_perform_http_callbacks() :: request:');
 ...
 // FIXME have hardcoded this to `get()` to temporarily solve mocking issues
    http_request.get(request, function (_err, _resp, _body) {
      ...
      self._debug_print('-----');
      self._debug_print('_perform_http_callbacks().http_request-callback() :: response:');
  ...
  */
  _poll_for_transmit: function (_callback) {
    ...
    self._debug_print('-----');
    self._debug_print('_poll_for_transmit() :: request:');
    self._debug_http_request(request);
    self._debug_print('-----');
    ...
    http_request.get(request, function (_err, _resp, _body) {
      ...
      self._debug_print('-----');
      self._debug_print('_poll_for_transmit().http_request-callback() :: response:');
      self._debug_http_response(_err, request, _resp, _body);
      self._debug_print('-----');
